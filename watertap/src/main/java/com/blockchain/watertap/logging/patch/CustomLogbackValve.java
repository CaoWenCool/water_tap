 ...
 * Copy from logback original source.
 */
 ...
 * @author zhangmengqi;
 */
public class CustomLogbackValve extends ValveBase implements Lifecycle, Context, AppenderAttachable<IAccessEvent>,
    ...
    static final String TEMP_PATH_KEY = "java.io.tmpdir";
    ...
    public void initLogaccess() {
        ...
        // 请注意 此处实现与logback原始实现不一致。优先直接从临时文件中获取
        File configFile = searchForConfigFileTomcatProperty(filename, TEMP_PATH_KEY);
        if (configFile == null) {
    ...
    private File searchForConfigFileTomcatProperty(String filename, String propertyKey) {
        ...
        if (propertyValue == null) {
            addInfo("System property \"" + propertyKey + "\" is not set. Skipping configuration "
                    + "file search with ${" + propertyKey + "} path prefix.");
            return null;
        }
    ...
    public void invoke(Request request, Response response) throws IOException, ServletException {
        try {
           ...
           CustomTomcatServerAdapter adapter = new CustomTomcatServerAdapter(request, response);
